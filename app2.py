...
try:
    from risk_engine import render_risk_warning_center
...
# 3. 侧边栏导航
# ==========================================
st.sidebar.title("舆擎｜企业经营风险AI预警平台")
st.sidebar.markdown("---")

st.sidebar.subheader("🎯 场景化分析配置")
st.sidebar.info("当前版本面向企业经营决策场景，聚焦融资、声誉、竞对三类核心风险。")
scenario_new = st.sidebar.selectbox(
    "选择业务场景：",
    ["经营风险预警", "品牌声誉监测", "竞对动态洞察"],
    key="scenario_new"
)
...
st.sidebar.markdown("---")
st.sidebar.info("🚀 **挑战杯省赛演示版**\n\n三大自研引擎分域训练：C-RATE 看关联，Anom-Graph 看传播，F-MOSAIC 看价格，面向企业关键经营时刻输出分级预警与处置方案。")
...
if menu == "📊 经营风险总览":
    ...
    if not df_news.empty and not df_scores.empty:
        ...
        col2.metric("覆盖重点企业", f"{df_scores['company'].nunique()} 家")
        ...
        with col_chart2:
            st.subheader("🔥 重点风险热度企业")
            top_companies = df_scores.groupby('company')['news_count'].sum().nlargest(10).reset_index()
            top_companies.columns = ['企业', '风险热度 (条)']
            fig_bar = px.bar(top_companies, x='企业', y='风险热度 (条)', 
                             color='风险热度 (条)', color_continuous_scale=HOT_COLOR_SCALE,
                             template="plotly_dark", text='风险热度 (条)')
        ...
        st.markdown("---")
        st.subheader("📋 企业经营风险量化清单")
        display_scores = df_scores.rename(columns={
            'date': '日期', 'company': '企业', 'news_count': '监测声量',
...
# ==========================================
    ...
    if not df_news.empty:
        ...
        selected_company = st.selectbox("请选择要分析的企业：", all_companies, 
                ...
                if ENGINE_AVAILABLE:
                    ...
                    if "error" in result:
                        ...
                        if 'dimension_scores' in result:
                            ...
                            # 如果没有分维度数据，生成默认模拟数据
                            if scenario_new == "经营风险预警":
                                dimension_scores = {"关联方风险": 72, "偿债能力": 76, "供应链稳定": 80, "行业政策": 78}
                            elif scenario_new == "品牌声誉监测":
                                dimension_scores = {"传播异常": 74, "信源可信": 78, "情绪突变": 73, "关联账号": 70}
                            elif scenario_new == "竞对动态洞察":
                                dimension_scores = {"价格变动": 75, "工商变更": 72, "招投标": 77, "舆情情绪": 74}
                            else:
                                dimension_scores = {"关联方风险": 72, "偿债能力": 76, "供应链稳定": 80, "行业政策": 78}
...
# ==========================================
    ...
    if RISK_ENGINE_AVAILABLE:
        render_risk_warning_center(st, px, df_news, df_scores, entity_label="企业")
    else:
        st.error("未找到 risk_engine.py，无法加载风险预警中心。")

elif menu == "📑 处置方案报告":
    st.title("📑 企业风险处置方案报告")
    st.markdown(f"基于【{scenario_new}】维度，生成可落地的分级处置方案，围绕“看懂风险 -> 判准等级 -> 方案直达”展开。")
    ...
    if not df_scores.empty:
        ...
        selected_company = st.selectbox("选择目标企业，一键生成处置方案：", all_companies,
                                        index=all_companies.index('宁德时代') if '宁德时代' in all_companies else 0)
        
        if st.button("🚀 生成处置方案报告", type="primary"):
            with st.spinner('正在汇总风险评分、触发原因与处置建议...'):
                # 研报也用CSV里的日期，保持一致
                ...
                # 场景化话术配置
                if scenario_new == "经营风险预警":
                    report_title = "C-RATE 经营风险预警报告"
                    focus_desc = "本报告侧重关联方风险、偿债能力、供应链稳定和行业政策影响"
                elif scenario_new == "品牌声誉监测":
                    report_title = "Anom-Graph 品牌声誉监测报告"
                    focus_desc = "本报告侧重传播异常、信源可信、情绪突变和关联账号扩散"
                elif scenario_new == "竞对动态洞察":
                    report_title = "F-MOSAIC 竞对动态洞察报告"
                    focus_desc = "本报告侧重价格变动、工商变更、招投标变化和舆情情绪"
                else:
                    report_title = "企业经营风险综合报告"
                    focus_desc = "本报告侧重企业关键业务场景下的风险识别和处置建议"
